...
Medical_Image_Viewer.viewer.Viewer.KEYCODE_CIRCLE = 82;
...
Medical_Image_Viewer.viewer.Viewer.prototype.drawViewer = function (force, skipUpdate) {
    var radiological = (this.container.preferences.radiological === "Yes"),
        ...
        showAnnotation = (this.container.preferences.showAnnotation === "Yes");
...
Medical_Image_Viewer.viewer.Viewer.prototype.drawScreenSlice = function (slice) {
    ...
    if (slice === this.surfaceView) {
        ...
        if (this.container.preferences.showRuler === "Yes" || this.container.preferences.showCircle === "Yes") {
            if (this.surfaceView === this.mainImage) {
                ...
                this.context.fillText(this.surfaceView.getCircleLength().toFixed(2), (slice.screenDim / 2) + textWidth - (offset / 2), Medical_Image_Viewer.viewer.Viewer.ORIENTATION_MARKER_SIZE + padding);
            }
        }
    } else {
...
// Collection to store annotations
var annotations = [];
Medical_Image_Viewer.viewer.Viewer.prototype.drawAnnotation = function () {
    var self = this;
    if (this.mainImage === this.surfaceView) {
        return;
    }
    
    // Add an input element to the body for text input
    ...
    // Mouse click event to show the input element for text annotation
    this.canvas.addEventListener("click", function (event) {
        var mouseX = event.clientX - self.canvas.getBoundingClientRect().left,
            mouseY = event.clientY - self.canvas.getBoundingClientRect().top;

        inputElement.style.left = mouseX + 280 + 'px';
        inputElement.style.top = mouseY + 20 + 'px';
        ...
        inputElement.onblur = function () {
            var annotationText = inputElement.value.trim();
            if (annotationText) {
                annotations.push({ text: annotationText, x: mouseX, y: mouseY });
                self.redrawAnnotations(annotations);
            }

            // Clear and hide the input element
            inputElement.value = '';
            inputElement.style.display = 'none'; 
...
Medical_Image_Viewer.viewer.Viewer.prototype.redrawAnnotations = function (annotations) {
    this.context.font = Medical_Image_Viewer.viewer.Viewer.ORIENTATION_MARKER_SIZE + "px sans-serif";
    this.context.strokeStyle = "#FF1493";
    this.context.fillStyle = "#FF1493";
    for (var i = 0; i < annotations.length; i++) {
        this.context.fillText(annotations[i].text, annotations[i].x, annotations[i].y);
    }
};
...
Medical_Image_Viewer.viewer.Viewer.prototype.drawCircle = function () {
    var text, metrics, textWidth, textHeight, padding, xText, yText;
    if (this.mainImage === this.surfaceView) {
        return;
    }

    // Extract common code for calculating circle coordinates
    function calculateCircleCoordinates(slice) {
        var circle1x = (slice.finalTransform[0][2] + (slice.circlePoints[0].x + 0.5) * slice.finalTransform[0][0]);
        var circle1y = (slice.finalTransform[1][2] + (slice.circlePoints[0].y + 0.5) * slice.finalTransform[1][1]);
        var circle2x = (slice.finalTransform[0][2] + (slice.circlePoints[1].x + 0.5) * slice.finalTransform[0][0]);
        var circle2y = (slice.finalTransform[1][2] + (slice.circlePoints[1].y + 0.5) * slice.finalTransform[1][1]);
        return { circle1x, circle1y, circle2x, circle2y };
    }

    var slice;
    if (this.mainImage === this.axialSlice) {
        slice = this.axialSlice;
    } else if (this.mainImage === this.coronalSlice) {
        slice = this.coronalSlice;
    } else if (this.mainImage === this.sagittalSlice) {
        slice = this.sagittalSlice;
    }

    var coordinates = calculateCircleCoordinates(slice);
    var circle1x = coordinates.circle1x;
    var circle1y = coordinates.circle1y;
    var circle2x = coordinates.circle2x;
    var circle2y = coordinates.circle2y;

    // Calculate circle properties
    var circleRadius = Medical_Image_Viewer.utilities.MathUtils.lineDistance(
        this.mainImage.circlePoints[0].x * this.mainImage.xSize,
        this.mainImage.circlePoints[0].y * this.mainImage.ySize,
        this.mainImage.circlePoints[1].x * this.mainImage.xSize,
        this.mainImage.circlePoints[1].y * this.mainImage.ySize
    ) / 2;

    // Calculate circle center
    var circleCenterX = (circle1x + circle2x) / 2;
    var circleCenterY = (circle1y + circle2y) / 2;

    // Initialize variables for interaction
    var isMoving = false;
    var isResizing = false;
    var initialMouseX, initialMouseY;
    var initialCircleRadius;

    // Event listener for mouse down to start moving or resizing
    this.canvas.addEventListener("mousedown", (event) => {
        var mouseX = event.clientX;
        var mouseY = event.clientY;

        // Check if the mouse is on the circle center
        if (isMouseOnCircleCenter(mouseX, mouseY)) {
            isMoving = true;
            initialMouseX = mouseX;
            initialMouseY = mouseY;
        }

        // Check if the mouse is on the circle boundary
        else if (isMouseOnCircleBoundary(mouseX, mouseY)) {
            isResizing = true;
            initialMouseX = mouseX;
            initialMouseY = mouseY;
    ...
    // Event listener for mouse move to move or resize the circle
    this.canvas.addEventListener("mousemove", (event) => {
        if (isMoving || isResizing) {
            ...
            if (isMoving) {
                circleCenterX += deltaX;
                circleCenterY += deltaY;
            }

            // Resize the circle
            else if (isResizing) {
                var distance = Math.sqrt(deltaX ** 2 + deltaY ** 2);
                circleRadius = initialCircleRadius + distance;
            }

            // Update initial mouse position for the next iteration
            initialMouseX = mouseX;
            initialMouseY = mouseY;
    ...
    // Event listener for mouse up to stop moving or resizing
    this.canvas.addEventListener("mouseup", () => {
        isMoving = false;
        isResizing = false;
    });

    // Function to check if the mouse is on the circle center
    function isMouseOnCircleCenter(mouseX, mouseY) {
        var distanceToCenter = Math.sqrt((mouseX - circleCenterX) ** 2 + (mouseY - circleCenterY) ** 2);
        return distanceToCenter <= circleRadius;
    }

    // Function to check if the mouse is on the circle boundary
    function isMouseOnCircleBoundary(mouseX, mouseY) {
        var distanceToCenter = Math.sqrt((mouseX - circleCenterX) ** 2 + (mouseY - circleCenterY) ** 2);
        var distanceToBoundary = Math.abs(distanceToCenter - circleRadius);
        return distanceToBoundary <= 5; // You can adjust the threshold for boundary detection
    }

    // Draw circular measurement
    this.context.setTransform(1, 0, 0, 1, 0, 0);
    this.context.strokeStyle = "#FF1493";
    this.context.fillStyle = "#FF1493";
    this.context.lineWidth = 2.0;
    ...
    // Draw circle
    this.context.arc(circleCenterX, circleCenterY, circleRadius, 0, 2 * Math.PI, false);
    ...
    text = Medical_Image_Viewer.utilities.StringUtils.formatNumber(Medical_Image_Viewer.utilities.MathUtils.lineDistance(
        ...
        this.mainImage.circlePoints[0].y * this.mainImage.ySize,
        this.mainImage.circlePoints[1].x * this.mainImage.xSize,
        this.mainImage.circlePoints[1].y * this.mainImage.ySize), false);

    metrics = this.context.measureText(text);
    textWidth = metrics.width;
    textHeight = 14;
    padding = 2;
    xText = parseInt((circle1x + circle2x) / 2) - (textWidth / 2);
    yText = parseInt((circle1y + circle2y) / 2) + (textHeight / 2);
...
Medical_Image_Viewer.viewer.Viewer.prototype.processParams = function (params) {
    ...
    if (!this.container.isDesktopMode()) {
        ...
        if (params.showAnnotation !== undefined) {
            this.container.preferences.showAnnotation = (params.showAnnotation ? "Yes" : "No");
        }
...
/*
    if (this.container.nestedViewer || ((Medical_Image_ViewerContainers.length > 1) && !this.container.collapsable)) {
        return;
    }
*/
    e = e || window.event;
...
Medical_Image_Viewer.viewer.Viewer.prototype.isShowingCircle = function () {
    return (this.container.preferences.showCircle === "Yes");
};
...
Medical_Image_Viewer.viewer.Viewer.prototype.isShowingAnnotation = function () {
    return (this.container.preferences.showAnnotation === "Yes");
};
