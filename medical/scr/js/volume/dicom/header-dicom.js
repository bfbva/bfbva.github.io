...
Medical_Image_Viewer.volume.dicom = Medical_Image_Viewer.volume.dicom || {};


/*** Constructor ***/
Medical_Image_Viewer.volume.dicom.HeaderDICOM = Medical_Image_Viewer.volume.dicom.HeaderDICOM || function () {
    this.series = null;
    this.seriesMap = [];
    this.error = null;
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.ORIENTATION_DEFAULT = "XYZ+--";
Medical_Image_Viewer.volume.dicom.HeaderDICOM.SUPPORTED_TRANSFER_SYNTAXES = [
    // uncompressed
    "1.2.840.10008.1.2",
    "1.2.840.10008.1.2.1",
    "1.2.840.10008.1.2.2",
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.isThisFormat = function (filename, data) {
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.setSeries = function (name, value) {
    var ctr;
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getDataScaleSlope = function (isElscint, image) {
    if (isElscint) {
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getDataScaleIntercept = function (isElscint, image) {
    if (isElscint) {
        return 0;
    }
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.finishedHeaderRead = function () {
    ...
    if (this.error) {
        ...
        this.dialogHandler.showDialog("Select DICOM Series", dialogData, this, Medical_Image_Viewer.utilities.ObjectUtils.bind(this, this.setSeries),
            Medical_Image_Viewer.utilities.ObjectUtils.bind(this, this.finishedSeriesSelection));
    } else {
        this.series = this.seriesMap[Object.keys(this.seriesMap)[0]];
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.isTransferSyntaxSupported = function () {
    var transferSyntax = this.series.images[0].getTransferSyntax();

    return (Medical_Image_Viewer.utilities.StringUtils.isStringBlank(transferSyntax) ||
        Medical_Image_Viewer.utilities.ArrayUtils.contains(Medical_Image_Viewer.volume.dicom.HeaderDICOM.SUPPORTED_TRANSFER_SYNTAXES,
            transferSyntax));
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.finishedSeriesSelection = function () {
    if (this.series.images.length > 0) {
        this.series.buildSeries();
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.readHeaderData = function (data, progressMeter, dialogHandler,
                                                                     onFinishedHeaderRead) {
    this.onFinishedHeaderRead = onFinishedHeaderRead;
    this.dialogHandler = dialogHandler;
    this.readNextHeaderData(data, 0, progressMeter, Medical_Image_Viewer.utilities.ObjectUtils.bind(this, this.finishedHeaderRead));
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.readNextHeaderData = function (data, index, progressMeter,
                                                                         onFinishedHeaderRead) {
    var image, series;

    if (index >= data.length) {
        progressMeter.drawProgress(1, "读取DICOM标头");
        onFinishedHeaderRead();
    } else {
        ...
        if (this.error) {
            ...
            progressMeter.drawProgress(index / data.length, "读取DICOM标头");
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getName = function () {
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getSeriesLabels = function () {
    return null;
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.findSeries = function (id) {
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.readImageData = function (progressMeter, onFinishedImageRead) {
    this.series.concatenateImageData(progressMeter, onFinishedImageRead);
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getImageDimensions = function () {
    ...
    if (this.series.isMosaic) {
        ...
        imageDimensions = new Medical_Image_Viewer.volume.ImageDimensions(parseInt(this.series.images[0].getCols() /
        ...
        imageDimensions = new Medical_Image_Viewer.volume.ImageDimensions(this.series.images[0].getCols(),
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getVoxelDimensions = function () {
    ...
    if (this.series.isMosaic || this.series.isMultiFrame) {
        voxelDimensions = new Medical_Image_Viewer.volume.VoxelDimensions(pixelSpacing[1], pixelSpacing[0], sliceSpacing,
        ...
        if ((this.series.images.length === 1) || daikon.Series.useExplicitOrdering) {
            ...
            voxelDimensions = new Medical_Image_Viewer.volume.VoxelDimensions(pixelSpacing[1], pixelSpacing[0], sliceDis,
                this.series.images[0].getTR() / 1000.0);
        }
    }
    ...
    voxelDimensions.spatialUnit = Medical_Image_Viewer.volume.VoxelDimensions.UNITS_MM;
    voxelDimensions.temporalUnit = Medical_Image_Viewer.volume.VoxelDimensions.UNITS_SEC;
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getImageType = function () {
    ...
    if (dataTypeDICOM === daikon.Image.BYTE_TYPE_INTEGER) {
        dataTypeCode = Medical_Image_Viewer.volume.ImageType.DATATYPE_INTEGER_SIGNED;
    } else if (dataTypeDICOM === daikon.Image.BYTE_TYPE_INTEGER_UNSIGNED) {
        dataTypeCode = Medical_Image_Viewer.volume.ImageType.DATATYPE_INTEGER_UNSIGNED;
    } else if (dataTypeDICOM === daikon.Image.BYTE_TYPE_FLOAT) {
        dataTypeCode = Medical_Image_Viewer.volume.ImageType.DATATYPE_FLOAT;
    } else if (dataTypeDICOM === daikon.Image.BYTE_TYPE_RGB) {
        dataTypeCode = Medical_Image_Viewer.volume.ImageType.DATATYPE_RGB;
    } else {
        dataTypeCode = Medical_Image_Viewer.volume.ImageType.DATATYPE_UNKNOWN;
    }

    it = new Medical_Image_Viewer.volume.ImageType(dataTypeCode, parseInt(this.series.images[0].getBitsAllocated() / 8),
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getImageRange = function () {
    ...
    imageRange = new Medical_Image_Viewer.volume.ImageRange(gMin, gMax);
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getOrientation = function () {
    var orientation = this.series.images[0].getOrientation();

    if (orientation === null) {
        orientation = Medical_Image_Viewer.volume.dicom.HeaderDICOM.ORIENTATION_DEFAULT;
    }

    // this fixes the cross-slice orientation sense (usually)
    orientation = orientation.substring(0, 5) + (this.series.sliceSense ? '+' : '-');

    return new Medical_Image_Viewer.volume.Orientation(orientation);
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getOrientationCertainty = function () {
    var orientation = this.series.images[0].getOrientation();

    if (orientation === null) {
        return Medical_Image_Viewer.volume.Header.ORIENTATION_CERTAINTY_UNKNOWN; // orientation could be found
    } else {
        if (this.series.isMosaic || this.series.isMultiFrameVolume) {
            return Medical_Image_Viewer.volume.Header.ORIENTATION_CERTAINTY_LOW;
        } else {
            return Medical_Image_Viewer.volume.Header.ORIENTATION_CERTAINTY_HIGH;
        }
    }
};
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getOrigin = function () {
    var m = this.getBestTransform();

    if (m) {
        var invm = numeric.inv(m);
        return new Medical_Image_Viewer.core.Coordinate(invm[0][3], invm[1][3], invm[2][3]);
    } else {
        return new Medical_Image_Viewer.core.Coordinate(0, 0, 0);
    }
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.hasError = function () {
    return (this.error !== null);
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getImageDescription = function () {
    ...
    return new Medical_Image_Viewer.volume.ImageDescription(notes.trim());
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getBestTransform = function () {
...
Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.getBestTransformOrigin = function () {
    return this.getOrigin();
};



Medical_Image_Viewer.volume.dicom.HeaderDICOM.prototype.toString = function () {
